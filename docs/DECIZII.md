...
| **D68** | Liniuțele „-" din textul brut primit → listă `<ul><li>` reală, cu marcaj **BULINĂ** (`•`), aceeași clasă (`.lista`) pentru AMBELE grupuri | Cerut explicit („schimbă liniuțele din text cu bulets"). **Corectat imediat, la feedback direct** („nu este bine, am cerut bulets nu linii"): prima livrare refolosise marcajul em-dash „—" moștenit din fosta listă `.exemple`, ca să nu introducă un al doilea limbaj vizual de marcaj pe pagină — raționament greșit aici, fiindcă cerința era explicită, iar marcajul se schimba oricum peste tot în secțiune. `::before`, nu `list-style`/`::marker`: `li` e `display: grid` (marcaj + text în două coloane, ca rândurile 2+ să se alinieze sub prima literă, nu sub bulină), iar un element grid nu mai randează marcajul nativ de listă în toate motoarele. Coloana de marcaj a scăzut de la 1.5rem la 1.1rem — bulina e mult mai îngustă decât em-dash-ul. | `S02Problema.astro` |
| **D69** | Secțiunea rămâne UN bloc continuu: fără linie de separare înainte de concluzie, fără propoziție finală la corp mare | **Reversează varianta livrată inițial în aceeași rundă.** Prima variantă pusese un `border-top` înainte de concluzie plus ultima propoziție la `--t-h2` bold, ca să păstreze dispozitivul „vacarm mic → claritate mare" din `tokens.css`. Feedback direct: „ce ți-am dat este o secțiune, nu trebuie divizată" — cele două, împreună, citeau ca începutul unei secțiuni NOI, nu ca închiderea acesteia. Concluzia curge acum din liste ca proză normală; singura diferență față de rândurile de listă e mărimea de corp (`--t-corp` vs `--t-corp-lg`): listele sunt enumerarea, concluzia e proza care le comentează. | `S02Problema.astro`, `copy.ts` |
...
| **D71** | „Iar întrebările care nu-ți dau pace sunt:" devine `h2`, la fel ca primul intro — și cele două perechi titlu+listă stau una lângă alta peste 52rem | Cerut direct („acest text trebuie să fie la nivelul «Simți că unele aspecte…». Echilibrează pagina și secțiunea"). Două consecințe: (1) e `h2` real, nu doar un paragraf care ARATĂ ca titlu — dacă arată ca titlu, trebuie să fie titlu și pentru un cititor de ecran; două `h2` în aceeași secțiune au deja precedent în pagină (`S04PentruCine.astro`). (2) Secțiunea declara `latime="larg"` (62rem) dar stivuia tot într-o coloană plafonată la `--max-proza` (38rem) — pe desktop rămânea un gol de ~40% în dreapta, în timp ce vecinele ei `larg` (PentruCine, Soluție, Rezultatul) își umplu lățimea cu grile. Acum o folosește și ea. Mărimea titlurilor a coborât de la `--t-statement` la `--t-h2`: pe două coloane, declarația mare (până la 3rem) umplea coloana cu 4-5 rânduri de titlu și făcea lista să pară notă de subsol. Verificat după: outline-ul paginii rămâne 1× `h1`, restul `h2`/`h3`, fără salturi. | `S02Problema.astro` |
...
## A doua rundă din 8 septembrie 2026 — §06 CeFacem retrasă, Lenis+GSAP eliminate

| # | Decizie | Motiv | Unde s-a aplicat |
|---|---|---|---|
| **D78** | „Cei cinci pași ai metodologiei" (`S06CeFacem.astro`) — RETRASĂ integral, împreună cu apparatus-ul Lenis+GSAP care exista strict pentru ea | Cerut explicit: „renunțăm la metodologie, secțiunea mai jos, pentru că explicăm cum ajungem la rezultat" — §05 Soluție acoperă deja același teritoriu, cu propria metodă de 6 pași (D73–D75, scrisă chiar în ziua precedentă). O secțiune separată care repeta aceeași idee, cu alți 5 pași diferiți, era dublură, nu întărire, și reversează D38 (decizia de a păstra metodologia separat de cele 10, confirmată atunci explicit prin `AskUserQuestion` — corectă la momentul ei, depășită acum de conținut). **Cascadă tehnică, nu doar ștergere de conținut:** GSAP ScrollTrigger trăia STRICT în §06 (regulă scrisă explicit așa în CLAUDE.md) — era SINGURUL consumator al lui pe toată pagina, iar Lenis fusese adoptat ÎMPREUNĂ cu GSAP special pentru pin/scrub-ul de-acolo (`lenis.on('scroll', ScrollTrigger.update)`, driven de `gsap.ticker`, nu de propriul RAF — Lenis nu putea rămâne „doar smooth-scroll" fără muncă suplimentară de recablare, neceruta de nimeni). Cu §06 retrasă, apparatus-ul întreg (~49KB gzip JS) rămânea orfan — eliminat, nu lăsat încărcat degeaba: `npm uninstall gsap lenis`, propul `motion` scos din `Base.astro` (interfață + scriptul de modul), `S06CeFacem.astro` șters (nu doar dezactivat), `ceFacem` scos din `copy.ts`. Pagina revine, de fapt, la poziția lui D29 („zero dependințe noi de motion") — nu prin reversare manuală, ci pentru că motivul care o depășise a dispărut. | `src/pages/index.astro`, `src/layouts/Base.astro`, `src/content/copy.ts`, `package.json`, `S06CeFacem.astro` (șters) |
| **D79** | Testele §06-specifice (pin/scrub GSAP, reduced-motion pentru pin) — șterse, nu adaptate; celelalte teste din `motion.spec.ts` care foloseau `#ce-facem` doar ca punct de scroll — reancorate pe `#facilitator` | Cele două teste GSAP nu mai au ce verifica (secțiunea nu există). Testele de buton flotant („apare după hero", mobil + desktop) foloseau `#ce-facem` incidental, ca prim punct convenabil de scroll după hero — nu ca subiect al testului; reancorate pe `#facilitator` (secțiunea imediat următoare acum), fără nicio schimbare de comportament testat. Selectorul de conținut vizibil fără JS (`#ce-facem .bloc`) a fost înlocuit cu `#solutie .pasi li` — listă echivalentă, din secțiunea care acoperă acum același rol. | `tests/e2e/motion.spec.ts`, `tests/copy-invariants.test.ts` (scoase referințele la `copy.ceFacem` din scanările agregate) |
...
## A doua rundă din 9 septembrie 2026 — §08 outro devine quickwin box, §09 retrasă parțial

| # | Decizie | Motiv | Unde s-a aplicat |
|---|---|---|---|
| **D82b** | Fostul `outro` din §08 (linie despărțitoare + text simplu) devine quickwin box | Cerut explicit. Refolosește `.card-depth` (aceeași sticlă mată ca biletul din Trust bar, cardurile din Agravare/Soluție) — nu un tratament vizual nou. Cele două propoziții rămân la ACEEAȘI greutate tipografică (rețeta lui `.sinteza` din Soluție: `--t-corp-lg`, 600, `--secundar`) — aceeași disciplină „nu X / ci Y" fără accent inventat (D69, D77). | `S04PentruCine.astro` |
| **D83** | §09 „Ce rezultat promitem" retrasă parțial: `h2` + lista de 4 livrabile + blocul „Ce nu îți promit" au dispărut; `cumLucram` și `chemaCineva` rămân | Cerut explicit: „secțiunea asta trebuie ștearsă", cu textul EXACT al h2+lista+granita citat ca „secțiunea" — `cumLucram`/`chemaCineva` nu au fost citate, și au primit, ÎN ACEEAȘI RUNDĂ, propriile instrucțiuni de editare (dovadă directă că rămân, nu doar absența unei mențiuni). **Semnalat, nu blocat:** `.granita` avea un comentariu „obligatoriu" în cod („singura formă de credibilitate care funcționează la un om saturat de promisiuni") și §2 din CLAUDE.md marca „Ce rezultat promitem" ca una din cele două secțiuni „cele mai importante" ale paginii — ambele semnalate lui Ciprian; cerința a fost menținută. **Consecință structurală**: fără `h2` propriu, primul heading rămas ar fi fost `cumLucram.h3` ca `h3` orfan, fără `h2` părinte în aceeași secțiune (regulă verificată obsesiv toată sesiunea, de la D66). Fix: `cumLucram.h3` („Cum lucrăm") a preluat rolul de `h2` al secțiunii; `chemaCineva.h3` rămâne `h3`, sub el. | `copy.ts` (`rezultatul`), `S03Rezultatul.astro` |
| **D84** | `cumLucram.lista` (6 întrebări) strânsă la 3, aceeași metodă ca D81 — plus o întrebare NOUĂ despre existența proceselor/procedurilor | Cerut explicit: „tot așa 3 puncte relevante" + „aș adăuga o întrebare de procese proceduri, dacă ele există sau nu în acea arie". Păstrate: întrebarea de deschidere („ce ai vrea să funcționeze diferit") și diagnosticul de cost concret („unde se consumă timp, bani, energie sau atenție"). Scoase: „cine este implicat" (acoperit deja de pasul 06 din §05 Soluție), „ce valoare ar avea schimbarea" (unghi apropiat de „unde se consumă"), „ce ar putea fi delegat sau construit diferit" (risca să sune ca alegere de soluție înainte de diagnostic, contrar poziționării §05). Noua întrebare — „Ce proceduri sau procese există deja și ce lipsește" — completează unghiul structural, lipsă din setul vechi. | `copy.ts` (`rezultatul.cumLucram.lista`) |
| **D85** | `chemaCineva` rescrisă: de la trei câmpuri (`corp` 3 propoziții + `accent` + `corpFinal` 2 propoziții, unghi „o singură persoană vede doar o parte din afacere") la UN paragraf, unghi nou „invitație ca gest de valoare" | Cerut explicit: „răspunsul trebuie dat într-un paragraf" + conținut dictat conversațional, nu citat literal — „formulează asta cu tonul din workshop... ton bun și prietenos, dar ține linia de business și valoare". Rescris cu skill-ul de copywriting (invocat explicit). O corectare de acuratețe pe parcurs: formularea primită spunea „invitat direct de speaker" — schimbat în „ai fost invitat", fără atribuire la o persoană anume, fiindcă pagina însăși spune că invitațiile circulă „în principal" prin distribuire (`trustBar.acces`, §1 CLAUDE.md), nu mereu direct de la Ciprian; o afirmație mai specifică decât atât ar fi riscat să nu fie adevărată pentru fiecare cititor — „pe pagina asta, un copy care minte e un bug" (CLAUDE.md §5). | `copy.ts` (`rezultatul.chemaCineva`), `S03Rezultatul.astro` |
| **D86** | Ultima secțiune de pe pagină rămasă pe marcaj em-dash (`cumLucram.lista`) migrată la bulină | §09 era chiar ultimul holdout — după D68 (§02), D76 (§05), D82 (§08), migrarea de marcaj e acum completă pe toată pagina. | `S03Rezultatul.astro` |

**Verificare:** `astro check` (0 erori), 139 teste unitare, `npm run contrast`, build, outline de
titluri (1× `h1`, restul `h2`/`h3`, fără salturi — verificat explicit: `h2 „Cum lucrăm"` →
`h3 „De ce să mai chemi pe cineva?"`), `motion.spec.ts` + `dialog-inscriere.spec.ts` 22/22 pe
...
## 9 septembrie 2026 — pregătire test end-to-end pe emailuri: nume vechi + calendar + bug de scoping

Cerut explicit: „add to calendar pe confirmarea de înscriere, la fel ca pe landing", plus
testarea reală a fiecărei situații care trimite email. Trei descoperiri, toate reparate
înainte de testul propriu-zis — nu avea rost să trimit emailuri reale cu bug-uri reale.

| # | Decizie | Motiv | Unde s-a aplicat |
|---|---|---|---|
| **D98** | Bug real, denumirea veche a produsului: „Prima Mutare spre un Asistent Digital" rămăsese hardcodată în subsolul comun de email, două subiecte de email și AMBELE pagini legale (Termeni + Confidențialitate) — plus exemplele din `docs/EMAILURI.md` și `docs/spec-tehnic-inscriere-16-09.md` | Pivotul de produs (2026-08-31, → „PRIMUL PAS") actualizase `src/content/copy.ts` (pagina), dar niciunul din aceste fișiere nu citea de-acolo — găsit la citirea codului înainte de a trimite emailuri de test, nu la rulare. Fix la rădăcină în `render.ts` (subsolul): `EVENIMENT.titlu` dinamic, nu un șir fix — un pivot viitor de produs nu mai poate uita locul ăsta, fiindcă nu-l mai atinge deloc. Subiectele din `templates.ts`, la fel. `.ics`-ul generat deja era corect (`src/lib/ics.ts` citea deja `EVENIMENT.titlu`) — doar exemplul din spec era stale. Cele trei teste din `tests/emails.test.ts` care afirmau numele VECHI ca „așteptat" au fost corectate, nu doar cele ocolite — validau un bug ca fiind comportament corect. | `src/emails/render.ts`, `src/emails/templates.ts`, `tests/emails.test.ts`, `legal/*.md`, `src/pages/confidentialitate.astro`, `src/pages/termeni.astro`, `docs/EMAILURI.md`, `docs/spec-tehnic-inscriere-16-09.md` |
| **D99** | „Adaugă în calendar" adăugat pe confirmarea IMEDIATĂ de înscriere (`/multumesc` + varianta în-dialog, cu JS) — extras într-o componentă comună (`LinkCalendar.astro`) | Cerut explicit. `/rezultat.astro` avea deja acest buton (pentru `reconfirmat`/`locRevendicat`, ajuns pe cale de link din email) — lipsea exact pe calea cea mai frecventă, imediat după submit. Al treilea loc care ar fi avut nevoie de markup+stil identic (după `/rezultat` și biletul din Trust bar) — extras, nu duplicat a treia oară, aceeași regulă care a scos deja `CardRaspuns.astro`. Apare doar pe stările care înseamnă „vii" (`inscris`, `reconfirmat`) — niciodată pe `anulatAnterior`, unde ar contrazice exact ce tocmai a citit omul. | `LinkCalendar.astro` (nou), `multumesc.astro`, `rezultat.astro` (refactorizat să-l refolosească), `DialogInscriere.astro` |
| **D100** | Bug real, PREEXISTENT, găsit la implementarea D99: `.confirmare-corp p` din confirmarea în-dialog nu se aplica NICIODATĂ — verificat computed style, randa negru implicit (`rgb(0,0,0)`), nu `--secundar` | `<p>`-urile din confirmarea în-dialog sunt create din JS (`document.createElement`), nu de Astro — n-au atributul de scope pe care Astro îl adaugă STRICT elementelor din șablon la build, iar compilatorul cere atributul pe FIECARE clasă dintr-un lanț de selector descendent, nu doar pe strămoș. Regula scrisă (`.confirmare-corp p { color: var(--secundar) }`) exista în cod de la runda de-atunci, dar n-a funcționat NICIODATĂ — un defect vizual subtil (negru vs. gri-teal închis, diferență mică) care a trecut nedetectat prin toate rundele anterioare de audit vizual. Fix: `.confirmare-corp :global(p)` — dezactivează scoparea STRICT pe bucata care țintește conținut creat din JS, cu strămoșul tot scopat normal. Aceeași cauză exista deja pentru linkul nou de calendar (D99) — reparat direct, nu descoperit separat a doua oară. Test nou, care prinde regresia dacă `:global()` dispare la o corectură viitoare: `toHaveCSS('color', 'rgb(47, 79, 79)')`. | `DialogInscriere.astro`, `tests/e2e/dialog-inscriere.spec.ts` |

**Verificare:** `astro check` (0 erori), 139 teste unitare, `npm run contrast`, build,
`dialog-inscriere.spec.ts` + `motion.spec.ts` + `formular.spec.ts` + `b2-siguranta-get.spec.ts`
